...
namespace ChapterReversalMod.Patches
{
    public static class StorylinePatch
    {
        ...
        public static class InitPatch1
        {
            ...
            {
                if (!__instance.transform.GetParents().Any(transform => transform.name == "[Panel]Invitation_Panel(Clone)")
                    || !__instance.transform.GetParents().Any(transform => transform.name == "[Script]StoryProgress")
